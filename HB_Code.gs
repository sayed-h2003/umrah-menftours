...
function ensureUsersSheet_() {
  ...
  if (!sh) {
    ...
    sh.appendRow(['admin', 'المدير', salt, hbHashPassword_('admin123', salt), 'admin', '{}', true, new Date()]);
  }
  return sh;
}

function hbHashPassword_(password, salt) {
...
function setSessionDurationMinutes(token, minutes) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', '', 'تغيير مدة الجلسة الافتراضية', '', minutes + ' دقيقة');
    return { ok: true };
...
function login(username, password) {
  try {
    ...
    if (hbHashPassword_(password, row[2]) !== row[3]) throw new Error('بيانات الدخول غير صحيحة');
    ...
    hbLogChange_(user, 'الدخول', user.username, 'تسجيل دخول', '', '');
...
function logout(token) {
  try {
    ...
    if (s) {
      ...
      hbLogChange_(s.username, 'الخروج', s.username, 'تسجيل خروج', '', '');
    }
    return { ok: true };
...
function changeOwnPassword(token, oldPassword, newPassword) {
  try {
    ...
    var row = sh.getRange(rowIdx, 1, 1, 8).getValues()[0];
    if (hbHashPassword_(oldPassword, row[2]) !== row[3]) throw new Error('كلمة المرور الحالية غير صحيحة');
    var newSalt = Utilities.getUuid();
    sh.getRange(rowIdx, 3, 1, 2).setValues([[newSalt, hbHashPassword_(newPassword, newSalt)]]);
    invalidateUserRecordCache_(user.username);
    hbLogChange_(user, 'المستخدمون', user.username, 'تغيير كلمة المرور الشخصية', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
// (hbLogChange_) عمدًا: تغيير عرض عمود بالسحب ليس "حدثًا" يستحق التوثيق في سجل التعديلات
...
function createUser(token, payload) {
  try {
    ...
    sh.appendRow([username, payload.displayName || username, salt, hbHashPassword_(payload.password, salt), role, JSON.stringify(perms), true, new Date()]);
    setUserAccountPerms_(username, payload.statementAccounts || []);
    invalidateUserRecordCache_(username);
    hbLogChange_(admin, 'المستخدمون', username, 'إنشاء مستخدم جديد (' + (role === 'admin' ? 'مدير' : 'مستخدم') + ')', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function updateUser(token, payload) {
  try {
    ...
    if (payload.password) {
      ...
      var newSalt = Utilities.getUuid();
      sh.getRange(rowIdx, 3, 1, 2).setValues([[newSalt, hbHashPassword_(payload.password, newSalt)]]);
    }
    setUserAccountPerms_(payload.username, payload.statementAccounts || []);
    invalidateUserRecordCache_(payload.username);
    hbLogChange_(admin, 'المستخدمون', payload.username, 'تعديل بيانات/صلاحيات مستخدم' + (payload.password ? ' (وإعادة تعيين كلمة المرور)' : ''), '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function logConfirmationIssued(token, resNo, confirmType, clientName) {
  try {
    ...
    hbLogChange_(actingUser, 'إصدار تأكيد حجز', resNo, '[' + typeLabel + '] طباعة/إصدار مستند تأكيد حجز', '', '',
...
function hbGetAppVersion() {
  return { version: HB_APP_VERSION, date: APP_VERSION_DATE };
}
...
function hbLogChange_(user, type, ref, desc, oldVal, newVal, extra) {
  try {
    var log = ensureChangelogSheet_();
  ...
  } catch (e) { Logger.log('hbLogChange_ error: ' + e.message); }
}
...
function markAllQaidEditFlagsReviewed(token) {
  try {
    ...
    rng.setValues(vals);
    if (count) hbLogChange_(user, 'حجوزات معدَّلة بعد القيد', '', 'تعليم ' + count + ' تنبيهًا كمُراجَع', '', '');
...
function showSourceSettingsDialog() {
  var tmpl = HtmlService.createTemplateFromFile('HB_App');
...
function editBookingFields(bookingKey, fieldsMap, token) {
  try {
    ...
    Object.keys(fieldsMap).forEach(function (colStr) {
      ...
      hbLogChange_(actingUser, 'تعديل حجز', bookingKey, 'تعديل "' + fieldLabel + '"', oldValue, val,
...
function editBookingField(bookingKey, colIndex1based, newValue, token) {
  try {
    ...
    hbLogChange_(actingUser, 'تعديل حجز', bookingKey, 'تعديل "' + fieldLabel + '"', oldValue, newValue,
      { hotelRef: hotelRefOrRef, clientName: clientName, recordKey: bookingRecordKey_(innerRefForLog) });
...
function downloadBackup() {
  ...
  var htmlOutput = HtmlService.createHtmlOutput('<html><body>' +
    ...
    '  google.script.run.hbDeleteTempFile("' + file.getId() + '");' +
    '  setTimeout(function() {' +
...
function hbDeleteTempFile(fileId) {
  try {
...
function hbDoGet_(e) {
  ...
  if (initialPage === 'portal') {
    try {
      return HtmlService.createHtmlOutputFromFile('HB_Portal')
...
function getPrintTemplate() {
  return HtmlService.createTemplateFromFile('HB_PrintTemplate').getRawContent();
}

function getConfirmationTemplate() {
  return HtmlService.createTemplateFromFile('HB_ConfirmationTemplate').getRawContent();
}

function getArrivalsReportTemplate() {
  return HtmlService.createTemplateFromFile('HB_ArrivalsReportTemplate').getRawContent();
}
...
function getBalancesReportTemplate() {
  return HtmlService.createTemplateFromFile('HB_BalancesReportTemplate').getRawContent();
}
...
function editClientPortalCode(token, clientName, newCode) {
  try {
    ...
    hbLogChange_(user, 'بوابة العملاء', rec.name, 'تعديل كود البوابة إلى: ' + newCode, rec.code, newCode);
...
function createClientPortal(token, clientName) {
  try {
    ...
    var salt = Utilities.getUuid();
    sh.appendRow([clientName, code, salt, hbHashPassword_(pw, salt), 'نشط', 'لا', new Date(), '', 'لا']);
    hbLogChange_(user, 'بوابة العملاء', clientName, 'إنشاء بوابة عميل (كود: ' + code + ')', '', '');
...
function resetClientPortalPassword(token, clientName) {
  try {
    ...
    var pw = genPortalString_(6, 'abcdefghjkmnpqrstuvwxyz23456789');
    var salt = Utilities.getUuid();
    ensurePortalSheet_().getRange(rec.rowIdx, 3, 1, 2).setValues([[salt, hbHashPassword_(pw, salt)]]);
    hbLogChange_(user, 'بوابة العملاء', rec.name, 'إعادة تعيين كلمة مرور البوابة', '', '');
...
function setClientPortalStatus(token, clientName, active) {
  try {
    ...
    hbLogChange_(user, 'بوابة العملاء', rec.name, active ? 'تفعيل البوابة' : 'تعطيل البوابة', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
function setClientPortalPriceVisibility(token, clientName, show) {
  try {
    ...
    hbLogChange_(user, 'بوابة العملاء', rec.name, show ? 'إظهار الأسعار للعميل' : 'إخفاء الأسعار عن العميل', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function setClientPortalShowPast(token, clientName, show) {
  try {
    ...
    hbLogChange_(user, 'بوابة العملاء', rec.name, show ? 'إظهار الحجوزات السابقة للعميل' : 'إخفاء الحجوزات السابقة', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
function deleteClientPortal(token, clientName) {
  try {
    ...
    hbLogChange_(user, 'بوابة العملاء', rec.name, 'حذف البوابة نهائيًا', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function portalLogin(code, password) {
  try {
    ...
    if (!rec || rec.status !== 'نشط' || hbHashPassword_(password, rec.salt) !== rec.hash) {
...
function portalChangePassword(ptoken, oldPassword, newPassword) {
  try {
    var rec = verifyPortalToken_(ptoken);
    if (hbHashPassword_(oldPassword, rec.salt) !== rec.hash) throw new Error('كلمة المرور الحالية غير صحيحة');
    if (!newPassword || newPassword.toString().length < 4) throw new Error('كلمة المرور الجديدة قصيرة جدًا (4 أحرف على الأقل)');
    var salt = Utilities.getUuid();
    ensurePortalSheet_().getRange(rec.rowIdx, 3, 1, 2).setValues([[salt, hbHashPassword_(newPassword, salt)]]);
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function appendBookingFromForm(token, payload) {
  try {
    ...
    hbLogChange_(user, 'حجز جديد', innerNo,
...
function registerBookingsBulk(token, rows) {
  try {
    ...
    results.forEach(function (res) {
      hbLogChange_(actingUser, 'حجز جديد', res.ref,
...
function logPortalClientEvent_(rec, req, desc, oldVal, newVal) {
  try {
    hbLogChange_('العميل: ' + (rec && rec.name ? rec.name : ''), 'طلب بوابة',
...
function portalSubmitBulkRequests(ptoken, list) {
  try {
    ...
    hbLogChange_('العميل: ' + rec.name, 'طلب بوابة', rec.name,
...
function respondPortalRequestAs_(user, requestId, action, editedData, message) {
  try {
    ...
    hbLogChange_(user, 'طلب بوابة', r.clientName, 'رد الموظف على طلب (' + r.type + ') → ' + r.status +
      (bkSummary ? ' — ' + bkSummary : '') +
...
function importLegacyPayments(sheetNames, token) {
  try {
    ...
    hbLogChange_(admin, 'استيراد دفعات قديمة', sheetNames.join(', '), 'استيراد ' + importedTotal + ' دفعة من ' + sheetsCount + ' ورقة', '', '');
...
function registerPayment(payment, token) {
  try {
    ...
    hbLogChange_(actingUser, 'دفعة', payment.partyName, '[تسجيل دفعة] ' + resolveDirectionLabel_(payment.direction) +
...
function registerLinkedPayment(payload, token) {
  try {
    ...
    // التعديلات ولا تُرسل تنبيه تليجرام إطلاقًا — سطرا hbLogChange_/tgEnqueue_ هما الإصلاح
    hbLogChange_(actingUser, 'دفعة', fromParty, '[دفعة مزدوجة] استلمنا من ' + fromParty + ' ودفعنا لـ' + toParty +
...
function registerPaymentsBatch(direction, rows, token) {
  try {
    ...
    notifyRows.forEach(function (nr) {
      hbLogChange_(actingUser, 'دفعة', nr.partyName, '[رفع مجمَّع] ' + dirLabel +
...
function registerManualEntry(partyName, date, type, amount, note, token) {
  try {
    ...
    hbLogChange_(actingUser, 'دفعة', partyName, '[بند يدوي] ' + dirLabel + (note ? (' — ' + note) : ''),
...
function updatePayment(id, fields, token) {
  try {
    ...
    invalidatePaymentsMemo_();
    if (diffs.length) {
      hbLogChange_(actingUser, 'دفعة', partyName,
...
function updateLinkedPayment(linkId, fields, token) {
  try {
    ...
    invalidatePaymentsMemo_();
    hbLogChange_(actingUser, 'دفعة', fromParty,
...
function deletePayment(id, token) {
  try {
    ...
    sh.deleteRow(rowIdx);
    hbLogChange_(actingUser, 'حذف دفعة', rowVals[1], '[حذف دفعة] ' + rowVals[2], rowVals[3], '',
...
function saveAccountProfile(token, payload) {
  try {
    ...
    hbLogChange_(user, 'بيانات حساب', name, 'تعديل بيانات حساب' + (renameResult ? (' (بعد إعادة تسمية من "' + oldName + '")') : '') + ' — النوع: ' + type + (role ? ' — الدور: ' + role : '') + (code ? (' — الكود: ' + code) : ''), '', '');
    return {
...
function generateAccountCodes() {
  ...
  Logger.log(summary);
  try { hbLogChange_('مدير النظام (تشغيل يدوي)', 'بيانات حساب', '', summary, '', ''); } catch (e) {}
  return summary;
}
...
function deleteAccount(name, token) {
  try {
    ...
    hbLogChange_(user, 'حذف حساب', name, 'حذف حساب "' + name + '" (بلا حركات)', name, '');
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e.message };
  }
...
function renameParty_(oldName, newName, actingUser) {
  ...
  if (bookingsUpdated > 0) {
    hbLogChange_(actingUser, 'إعادة تسمية طرف', oldName, bookingsUpdated + ' حجز تأثر بإعادة التسمية', oldName, newName);
    invalidateSourceCache_();
  }
...
// ==========================================================
// وضع السحب (Polling) — الحل الجذري لعاصفة hbDoPost_
// ==========================================================
// المشكلة الجذرية: مع الويب هوك، أي بطء في hbDoPost_ (بسبب ازدحام أو خطأ 302) يجعل تليجرام
// يُعيد إرسال نفس التحديث مرارًا، فتتكاثر تنفيذات hbDoPost_ وتُشبع حصة تنفيذ الحساب بالكامل،
// فيبطؤ كل شيء (كشف الحساب، الجلسة، الحفظ). الحل: حذف الويب هوك نهائيًا والاعتماد على
// getUpdates من مشغّل زمني — لا hbDoPost_ إطلاقًا، فلا عاصفة. السعر: زمن استجابة حتى دقيقة،
...
// المعالج الموحّد لأي تحديث — يُستدعى من hbDoPost_ (وضع الويب هوك) ومن tgPollUpdates_ (وضع
...
function saveTelegramSettings(token, cfg, botToken) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', 'تليجرام',
...
// ==========================================================
var GEMINI_PROP_KEY1_ = 'HB_GEMINI_API_KEY_1';
var GEMINI_PROP_KEY2_ = 'HB_GEMINI_API_KEY_2';
...
function setGeminiKeys(token, key1, key2) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', 'Gemini', 'تحديث مفاتيح الذكاء الاصطناعي لقراءة التأكيدات', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function clearTelegramToken(token) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', 'تليجرام', 'حذف توكن بوت تليجرام وإيقاف التنبيهات', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
// ==========================================================
...
function telegramDiagnose(token) {
  try {
    ...
    var mode = tgBotMode_();
    if (mode === 'polling') {
      add('وضع الاستقبال', true, 'وضع السحب الموثوق (Polling) — لا ويب هوك، لا عاصفة hbDoPost_');
      try {
...
// Apps Script لا يتيح قراءة ترويسات الطلب إطلاقًا في hbDoPost_ — فالسبيل الوحيد للتحقق أن
// الطلب من تليجرام فعلًا هو سرّ داخل الرابط المسجَّل بـsetWebhook (e.parameter.tghook).
//
// ولماذا ربط الهوية إجباري؟ hbDoPost_ ينفَّذ دائمًا بصلاحيات مالك النشر ولا توجد جلسة متصفح،
...
function generateTelegramLinkCode(token, username) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', username, 'توليد كود ربط بوت تليجرام', '', '');
...
function tgConsumeLinkCode_(code, tgId, tgName) {
  ...
  invalidateTgUsersCache_();
  hbLogChange_('بوت تليجرام', 'الإعدادات', rec.username,
...
// تُخزَّن في ورقة لا في الذاكرة: كل نداء hbDoPost_ تنفيذ مستقل تمامًا في Apps Script، فلا
...
// سجل نشاط البوت — انتقل من الكتابة في شيت (appendRow في كل تحديث كان يُبطئ hbDoPost_ فيدفع
...
function tgLogToChangelog_(chatType, who, text, result) {
  ...
  hbLogChange_(String(who || 'مستخدم تليجرام'), 'تليجرام',
...
// **hbDoPost_ يجب ألا تُرجع أي محتوى إطلاقًا — لا ContentService ولا HtmlService.**
...
function hbDoPost_(e) {
  ...
  try {
    ...
    Logger.log('hbDoPost_: ' + err.message);
...
function tgIssueStatementDoc_(chatId, from, ctx, chatType, party, format) {
  ...
  hbLogChange_(ctx.user, 'كشف حساب', party, 'إصدار كشف حساب ' + (format === 'image' ? 'صورة' : 'PDF') + ' عبر بوت تليجرام', '', '',
...
function tgConfirmIssue_(chatId, from, ctx, chatType, format) {
  ...
  tgSendDocFormat_(target.id, format, html, typeLabel + ' - ' + b.innerRef + ' - ' + fileStamp,
    ...
    '\nأصدره: ' + tgEsc_(staffDisplayName_(ctx.user)) + ' · عبر البوت');
  hbLogChange_(ctx.user, 'إصدار تأكيد حجز', b.innerRef,
...
//   • لكن 302 ليس حتميًا في Apps Script: هو يظهر *فقط* حين تُرجع hbDoPost_ محتوى
//     (ContentService/HtmlService)، لأن التحويل موجود أصلًا لتسليم جسم الرد. حين لا تُرجع
//     hbDoPost_ شيئًا، يردّ Apps Script بـ 200 مباشرة بلا تحويل — وهذا ما يفعله بوت يعمل بلا
//     عطل في مشروع Apps Script آخر (كل مسارات hbDoPost_ فيه "return;" مجردة).
// لذلك 302 هنا = دليل على أن النشر المُفعَّل ما زال يشغّل نسخة قديمة من hbDoPost_ تُرجع محتوى،
    ...
    if (out.code === 301 || out.code === 302 || out.code === 307 || out.code === 308) {
      ...
      try {
        ...
        out.verdict = '⛔ الرابط يردّ 302 في القفزة الأولى — تليجرام سيرفض كل تسليم ويُعيد المحاولة بلا توقف.\n' +
          'السبب المؤكَّد: النشر المُفعَّل ما زال يشغّل نسخة قديمة من الكود تُرجع فيها hbDoPost_ محتوى (ContentService). ' +
          'النسخة الحالية لا تُرجع شيئًا من hbDoPost_ إطلاقًا، وهو ما يجعل الرد 200 مباشرة بلا تحويل.\n' +
...
function tgWebhookSelfHeal_() {
  ...
  try {
    ...
    // قديم تُرجع فيه hbDoPost_ محتوى — وذلك يحتاج نشر إصدار جديد بيد المدير، ويظهر صريحًا في
...
function registerTelegramWebhook(token, baseUrl) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', 'تليجرام', 'تسجيل ويب هوك بوت تليجرام (تفعيل الأوامر التفاعلية)', '', url);
    return safeReturn_({
...
function unregisterTelegramWebhook(token) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', 'تليجرام', 'إيقاف الأوامر التفاعلية (حذف الويب هوك)', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function enableTelegramPolling(token) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', 'تليجرام', 'تفعيل وضع السحب الموثوق (Polling) — حذف الويب هوك', '', 'polling');
...
function saveTelegramBotUser(token, tgId, active, perms) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', u.username, 'تعديل صلاحيات بوت تليجرام لـ' + u.username,
...
function deleteTelegramBotUser(token, tgId) {
  try {
    ...
    invalidateTgUsersCache_();
    hbLogChange_(admin, 'الإعدادات', u.username, 'فكّ ربط حساب تليجرام عن ' + u.username, u.tgId, '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function createTelegramPendingLink(token, username, idType, idValue) {
  try {
    ...
    hbLogChange_(admin, 'الإعدادات', username,
...
function deleteTelegramPendingLink(token, id) {
  try {
    ...
    tgSavePendingLinks_(list);
    hbLogChange_(admin, 'الإعدادات', '', 'حذف ربط تليجرام مُعلَّق', '', '');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function tgIssueArrivalsDoc_(chatId, from, ctx, chatType, d, format) {
  ...
  hbLogChange_(ctx.user, 'كشف وصول', d.client, 'إصدار كشف وصول (' + (format === 'image' ? 'صورة' : 'PDF') + ') عبر بوت تليجرام', '', '',
...
function tgApplyBookingPrices_(ctx, bookingKey, entry) {
  ...
  if (diffs.length) {
    invalidateSourceCache_();
    hbLogChange_(ctx.user, 'تعديل حجز', bookingKey, 'تسجيل أسعار ناقصة عبر بوت تليجرام', '', '',
...
function tgNewBookingIssue_(chatId, from, d, skipDupCheck) {
  ...
  try {
    ...
    hbLogChange_(ctx.user, 'حجز جديد', innerNo,
...
function tgNewBookingUpdateExisting_(chatId, from, d, dupKey) {
  ...
  try {
    ...
    hbLogChange_(ctx.user, 'تعديل حجز', dupKey,
...
function editBookingFieldsAsUser_(actingUser, bookingKey, fieldsMap) {
  try {
    ...
    Object.keys(fieldsMap).forEach(function (colStr) {
      ...
      count++;
      var fieldLabel = BOOKING_COL_LABELS[col - 1] || ('عمود ' + colLetter_(col - 1));
      hbLogChange_(actingUser, 'تعديل حجز', bookingKey, 'تعديل "' + fieldLabel + '" عبر بوت تليجرام', oldValue, val,
        { hotelRef: hotelRefOrRef, clientName: clientName, recordKey: bookingRecordKey_(innerRefForLog) });
      if (String(oldValue || '') !== String(val || '')) {
        diffs.push({ label: fieldLabel, oldVal: oldValue, newVal: val });
        if (QAID_EDIT_TRIGGER_COLS_1BASED_[col]) sigChanged_.push(fieldLabel);
...
function tgOcrCommitUpdate_(chatId, from, data, bookingKey) {
  ...
  try {
    ...
    hbLogChange_(ctx.user, 'تعديل حجز', bookingKey, 'تحديث الحجز من تأكيد فندق (تليجرام/OCR)' +
